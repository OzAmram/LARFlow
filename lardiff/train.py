...
from lardiff import data_loader, flow_matching, lar_data, transformer, util


def get_total_norm(tensors) -> torch.Tensor:
    # torch.nn.utils.get_total_norm only exists in torch >= 2.6
    norms = [torch.linalg.vector_norm(t.detach()) for t in tensors]
    if not norms:
...
class Trainer:
    def __init__(
        ...
        self.train_loader, self.val_loader, self.trafos = lar_data.get_data_loaders(
            conf["data"],
            self.batch_size,
            self.rank,
            self.world_size,
